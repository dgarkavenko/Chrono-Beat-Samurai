...
public class UniBpmAnalyzer
{
    ...
    public static int AnalyzeBpm(AudioClip clip, float startSeconds, float endSeconds)
{
    // Ensure start and end times are within the clip's duration
    if (startSeconds < 0f) startSeconds = 0f;
    if (endSeconds < 0f || endSeconds > clip.length) endSeconds = clip.length;

    // Early exit if the clip is null
    if (clip == null)
    {
    ...
    int frequency = clip.frequency;
    int channels = clip.channels;
    int startSample = Mathf.FloorToInt(startSeconds * frequency);
    int endSample = Mathf.FloorToInt(endSeconds * frequency);

    // Ensure we have a valid range of samples
    if (startSample < 0 || startSample >= clip.samples * channels ||
        endSample < 0 || endSample > clip.samples * channels ||
        startSample >= endSample)
    {
        Debug.LogError("Invalid sample range for BPM analysis.");
        return -1;
    }
    ...
    // Get sample data from the specified range
    var sampleData = new float[sampleCount];
    clip.GetData(sampleData, startSample);

    // Create volume array from the specified sample data
    var volumeArr = CreateVolumeArray(sampleData, frequency, channels, splitFrameSize);

    // Search bpm from volume array
    int bpm = SearchBpm(volumeArr, frequency, splitFrameSize);

    // Output results for debugging
    Debug.Log($"Analyzed BPM from {startSeconds}s to {endSeconds}s: {bpm}");
